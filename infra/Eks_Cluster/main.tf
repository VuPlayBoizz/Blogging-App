...
module "vpc-nat" {
    ...
    vpc_id              = module.vpc.vpc_id
    public_subnet_1b_id = module.vpc-subnet-1b.public_subnet_id
    tags                = var.tags
}
...
module "eks-cluster" {
    source = "./Modules/10_aws_eks"
    cluster_name            = var.cluster_name
    eks_cluster_role_arn    = module.iam-roles.eks_cluster_role_arn
    kubernetes_version      = var.kubernetes_version
    private_subnet_ids      = [module.vpc-subnet-1a.private_subnet_id, module.vpc-subnet-1b.private_subnet_id]
    control_plane_sg_id     = module.vpc-security-group.control_plane_sg_id
    tags                    = var.tags
}

module "eks-worker-node" {
    source = "./Modules/11_aws_worker_nodes"
    cluster_name                = var.cluster_name
    ...
    worker_node_sg_id           = module.vpc-security-group.worker_node_sg_id
    depends_on                  = [module.eks-cluster]
}

module "ebs-csi-driver" {
    source = "./Modules/12_aws_addons"
    cluster_name = var.cluster_name
    depends_on = [module.eks-cluster,
                  module.eks-worker-node]
}
